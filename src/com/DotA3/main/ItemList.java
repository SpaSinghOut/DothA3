...
public class ItemList{
	...
	private int[] attributes = {Constants.bonusStrength, Constants.bonusAgility, 
	Constants.bonusIntelligence, Constants.allAttributesBonus};
	private Item[] items;
	Type type;
	Alive owner;
	...
	public void addItem(int slot, Item item){
		if(items[slot - 1] == null)	{
			...
			((Player)((Hero)owner).owner).gui.out("Slot number: " + slot + " is full. Attemting to add to first free slot");
			if(addIntoNextFirstSlot(item) == 0)((Player)((Hero)owner).owner).gui.out("Unable to add item to inventory. Inventory is full");
			return;
		}
		switch(type){
		case STASH:
		...
		((Player)((Hero)owner).owner).gui.out("added item into slot: " + slot);
	}
