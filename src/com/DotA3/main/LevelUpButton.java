...
public class LevelUpButton extends StateChangingButton implements MouseListener{
	...
	public void update(){
		if(state == State.IDLE && owner.owner.hero.getStat(Constants.abilityPoints) > 0)setState(State.USE);
		String string = " Level up: " + owner.owner.hero.getStat(Constants.abilityPoints) + "!";
		setText(state == State.LEVEL?"choose ability" : string);
		if(owner.owner.hero.getStat(Constants.abilityPoints) > 0)setVisible(true);
		else {
			setVisible(false);
	...
	public void setState(State setState){
		if(setState == State.LEVEL && !(this.owner.owner.hero.getStat(Constants.abilityPoints) > 0))return;
